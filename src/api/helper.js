    ...
    transformResponse: [
      function (data) {
        try {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            Object.keys(data2[0].attributes).map((key, index) => {
              ...
              if (data2[0].attributes[key].nodeName == 'src') {
                tmp2.Img =
                  (data2[0].attributes[key].value[0] == 'h' ? '' : 'https:') +
                  data2[0].attributes[key].value;
              }
            });
            ...
            Object.keys(data3[0].attributes).map((key, index) => {
              ...
              if (data3[0].attributes[key].nodeName == 'src') {
                tmp2.UserInfo.AvatarUrl =
                  (data3[0].attributes[key].value[0] == 'h' ? '' : 'https:') +
                  data3[0].attributes[key].value;
              }
            });
            const data4 = items[i].getElementsByClassName('card', false);
            Object.keys(data4[0].childNodes[0].nextSibling.attributes).map(
              (key, index) => {
                if (
                  data4[0].childNodes[0].nextSibling.attributes[key].nodeName ==
                  'href'
                ) {
                  tmp2.Id =
                    data4[0].childNodes[0].nextSibling.attributes[key].value;
                }
              },
            );
            const data5 = items[i].getElementsByClassName(
              'feed__justification',
              false,
            );
            Object.keys(data5[0].childNodes[0].nextSibling.attributes).map(
              (key, index) => {
                if (
                  data5[0].childNodes[0].nextSibling.attributes[key].nodeName ==
                  'href'
                ) {
                  tmp2.UserInfo.Id =
                    data5[0].childNodes[0].nextSibling.attributes[key].value;
                }
              },
            );
            tmp.push(tmp2);
          ...
          let nextPage = '';
          Object.keys(data6[0].childNodes[0].nextSibling.attributes).map(
            (key, index) => {
              if (
                data6[0].childNodes[0].nextSibling.attributes[key].nodeName ==
                'href'
              ) {
                nextPage =
                  data6[0].childNodes[0].nextSibling.attributes[key].value;
              }
            },
          );
    ...
    transformResponse: [
      function (data) {
        try {
          ...
          const parsed = parser.parseFromString(data, 'text/html');
          const dataSoluong = parsed.getElementsByClassName(
          ...
          for (let i = 0; i < items.length; i++) {
            let tmp2 = {
              Img:
                'https://assets-global.cpcdn.com/assets/blank_logo-66f12ec6151fd5055601867856c705c89750e759667b4b2da3967c4eba2b3e5b.png',
              Name: '',
              Id: '',
              Source: 'cookpad',
              UserInfo: {
            ...
            const InfoUser = items[i].getElementsByClassName(
              'rounded-full',
              false,
            );
            Object.keys(InfoUser[0].attributes).map((key, index) => {
              ...
              if (InfoUser[0].attributes[key].nodeName == 'src') {
                tmp2.UserInfo.AvatarUrl =
                  InfoUser[0].attributes[key].value[0] == 'h'
                    ? InfoUser[0].attributes[key].value
                    : 'https:' + InfoUser[0].attributes[key].value;
              }
            });
            const title = items[i].getElementsByClassName(
              'recipe-title',
              false,
            );
            ...
            const hinhanh = items[i].getElementsByClassName(
              'lazy-image',
              false,
            );
            if (hinhanh.length) {
        ...
        //console.log(data4[0].childNodes[0].nextSibling.childNodes[0].data)//first page*/
          ...
          const data5 = parsed.getElementsByAttribute(
            'class',
            'pagination__next',
          );
          let nextPage = '';
    ...
    transformResponse: [
      function (data) {
        try {
          ...
          const parsed = parser.parseFromString(data, 'text/html');
          const dataDesc = parsed.getElementsByClassName(
            'user-header__profile-stats-item',
            false,
          );
          const userDesc = {
            mon: dataDesc[0].childNodes[0].nextSibling.childNodes[0].data, // + ' món',
            nguoiquantam:
              dataDesc[1].childNodes[0].nextSibling.childNodes[0].data, // + ' người quan tâm',
            banbep: dataDesc[2].childNodes[0].nextSibling.childNodes[0].data, // + ' bạn bếp'
          };
          const thongtinUser = parsed.getElementsByClassName(
            'user-header__avatar',
            false,
          );
          let userInfo = {
          ...
          for (let i = 0; i < data2.length; i++) {
            ...
            const title = data2[i].getElementsByClassName(
              'recipe-title',
              false,
            );
            tmp2.Name = title[0].childNodes[0].nextSibling.childNodes[0].data;
            ...
            const hinhanh = data2[i].getElementsByClassName(
              'lazy-image',
              false,
            );
            if (hinhanh.length) {
          ...
          const data3 = parsed.getElementsByClassName(
          ...
          Object.keys(data3[0].attributes).map((key, index) => {
            if (data3[0].attributes[key].nodeName == 'style') {
              bgUer = data3[0].attributes[key].value
